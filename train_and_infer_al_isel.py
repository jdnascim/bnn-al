...
from src.gnn.pseudo_labeling import pl_update
from src.arch.utils import read_setup
from src.al.al import al_update
from src.gnn.utils import generate_graph_hetero, run_base, generate_graph, validate_best_model
import torch
import os
...
if __name__ == "__main__":
    ...
    parser.add_argument("--al_iter", default=2, type=int)
    parser.add_argument("--al_batch", default=16, type=int)
    parser.add_argument("--al_isel", default="random", type=str, required=False)
    parser.add_argument("--aug_unlbl_set", action="store", type=str, default=None)
    ...
    parser.add_argument("--threshold_cluster", default=0, type=int)
    ...
    if hetero is True:
        model = BayesianHeteroGNN(**kwargs)
    ...
    elif arch_setup_data["group"] == "gnn-base":
        model = BaseGNN(**kwargs)
    elif arch_setup_data["group"] == "mlp-base":
        model = BaseMLP(**kwargs)
    
    print(kwargs)
    model = model.to(device)
    
    if hetero is True:
        pyg_graph_train, pyg_graph_dev = generate_graph_hetero(**kwargs)
    else:
    ...
    for i in range(args.al_iter + 1):
        ...
        with open(resf, 'r') as file:
            data_dict = json.load(file)
    
        end_time = datetime.now()
        runtime = end_time - start_time
            
        data_dict['runtime'] = str(runtime.seconds)
    
        with open(resf, 'w') as file:
            json.dump(data_dict, file)
        
        start_time = datetime.now()
    
        if args.pl is not None and i < al_iter:
            model = best_model
            
            pyg_graph_train, pyg_graph_dev = pl_update(model, pyg_graph_train, pyg_graph_dev, lbl_num=labeled_size, **kwargs)
        ...
        if args.al is not None and i < al_iter:
            ...
            pyg_graph_train, pyg_graph_dev = al_update(model, pyg_graph_train, pyg_graph_dev, lbl_num=labeled_size, **kwargs)
