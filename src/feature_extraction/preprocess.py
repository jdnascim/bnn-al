...
def remove_emoji(text):
    ...
    for em in emoji_info:
        positions.append([em["match_start"], em["match_end"]])

    positions.sort(reverse=True)  # Sort positions in descending order to avoid shifting indices
    for start, end in positions:
        text = text[:start] + text[end:]
    return text

URL = re.compile(r'((([A-Za-z]{3,9}:(?:\/\/)?)(?:[\-;:&=\+\$,\w]+@)?[A-Za-z0-9\.\-]+|(?:www\.|[\-;:&=\+\$,\w]+@)[A-Za-z0-9\.\-]+)((?:\/[\+~%\/\.\w\-_]*)?\??(?:[\-\+=&;%@\.\w_]*)#?(?:[\.\!\/\\\w]*))?)')
